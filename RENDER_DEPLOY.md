...
6. **Парсинг на Render сильно медленнее, чем локально**
   ...
   - стартовые безопасные настройки для Render:
     ...
     - `APP_PARSER_REQUEST_TIMEOUT_SECONDS=30`
...
7. **Ошибка `No such file or directory: 'curl.exe'`**
   - это fallback-ветка после timeout/ошибки запроса.
   - в Linux-контейнере должен использоваться `curl` (не `curl.exe`); обновите сервис до последних коммитов.
   - если после обновления остаются timeouts, увеличьте `APP_PARSER_REQUEST_TIMEOUT_SECONDS` до `40-45`.
